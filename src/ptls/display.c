...
void displayHelp() {
    ...
    printHelpLine(IGNORE_BACKUPS_ARG, IGNORE_BACKUPS_ARG_W,
                  IGNORE_BACKUPS_ARG_DESC);
    ...
    printHelpLine(NUMERIC_UID_GID_ARG, NUMERIC_UID_GID_ARG_W,
                  NUMERIC_UID_GID_ARG_DESC);
...
void displayList(Fields **fields, int count, ColumnSizes *cSize, Args *args,
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (args->inode)
            printField(fields[i]->inode, strlen(fields[i]->inode),
                       cSize->inode);
        ...
        printField(fields[i]->hardlinks, strlen(fields[i]->hardlinks),
                   cSize->hardlinks);
